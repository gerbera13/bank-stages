...
export function extractPlanVector(imageData) {
  ...
  const layout = (usedFlights) => {
    ...
    // Окно бывает только в НАРУЖНОЙ стене, дверь между помещениями — только
    // во внутренней. Разбор «комната по обе стороны — дверь» на это не смотрит
    // и ошибается в обе стороны: разрыв во внутренней стене, чьи грани не
    // спарились, с одной стороны упирается в щель внутри самой стены и уходит
    // в окна (на БТИ так набирался десяток окон посреди здания), а оконный
    // проём в наружной стене, наоборот, читается дверью. Разводим по контуру.
    const edge = Math.min(vec.w, vec.h) * 0.04
    const onOutline = (o) => outline && distToPoly(outline, o.x, o.y) <= edge
    return {
      rooms,
      outline,
      doors: doors.filter((o) => !onOutline(o)),
      windows: outline
        ? windows.filter(onOutline).concat(doors.filter(onOutline))
        : windows,
    }
  }
...
export function toRawBlueprintVector(v, name = 'Конвертер: план из чертежа') {
  ...
  for (const f of v.flights) {
    ...
    // Наружная лестница на план НЕ выносится. Раньше она рисовалась круглым
    // значком за контуром здания — крыльцо превращалось в оранжевый кружок,
    // висящий рядом с домом, и читалось не как лестница, а как пометка. Марш
    // мы всё равно находим и используем (по нему считается зона, из которой
    // убираются ступени), но показывать его нечем: снаружи здания у плана
    // нет ни стен, ни пола, к которым его привязать.
  }
