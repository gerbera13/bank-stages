...
export function vectorizeWalls(imageData) {
  ...
  // Сеть стен здания — ОДНА связная фигура во весь лист: на трёх проверочных
  // чертежах её габарит 741×297, 485×353 и 637×264 при размерах кадра 782×352,
  // 528×431 и 691×317. Мелкий кусок, лежащий отдельно от неё, стеной быть не
  // может — это подпись, мебель или прибор.
  //
  // Так подпись «офис 8» (кусок 35×13) перестаёт резать кабинет пополам: по
  // числу соседей её не поймать, рядом стоит всего одна цифра «8», — а по
  // связности видно сразу. Ступени лестницы при этом не страдают: они тонкие
  // (18×2 против 35×13), и короткая сторона их спасает. Раньше эта правка
  // роняла лестницу, но по другой причине — марш держался на случайной
  // диагонали Хафа; теперь ступени ищутся прямо по чернилам и не зависят от
  // того, что здесь отброшено.
  {
    const spanOf = (c) => Math.max(c.w, c.h)
    let main = null
    for (const c of comps) if (!main || spanOf(c) > spanOf(main)) main = c
    const loose = Math.round(Math.min(w, h) * 0.15)
    if (main) {
      for (const c of comps) {
        if (c === main) continue
        if (spanOf(c) <= loose && Math.min(c.w, c.h) >= 6) drop.add(c)
      }
    }
  }
